...
class web_fifo_server(orm.Model):
    ...
    def _get_fifo_server_from_user(self, cr, uid, context=None):
        ...
        if user.fifo_server_id.id:
            ...
            if not fifo_server_ids:
                raise orm.except_orm(_('Error:'), _("No FiFo server configured for the company '%s'.") % user.company_id.name)
            else:
	...
	def _connect_to_fifo(self, cr, uid, context=None):
		...
		if not user.fifo_username or user.fifo_password:
			raise orm.except_orm(_('Error:'), _('No FiFo details specified for the current user.'))

		try:
			wiggle = Wiggle()
			wiggle.init(fifo_server.host, user.fifo_username, user.fifo_password, user.fifo_token)
			if wiggle.get_token():
				user.write(cr, uid, {'fifo_token': wiggle.get_token()}, context=context)
		except Exception, e:
			_logger.error("Error connecting to Wiggle server %s" % fifo_server.host)
			_logger.error("Details: %s" % e.strerror)
			raise orm.except_orm(_('Error:'), _("Problem in the request from OpenERP to FiFo. Details: %s" % e.strerror))
			return False
